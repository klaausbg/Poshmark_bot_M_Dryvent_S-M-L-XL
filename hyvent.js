...
const POSHMARK_URL =
  "https://poshmark.com/search?query=the%20north%20face%20dryvent&sort_by=added_desc&department=Men&category=Jackets_%26_Coats&brand%5B%5D=The%20North%20Face&size%5B%5D=S&size%5B%5D=L&size%5B%5D=M&size%5B%5D=XL&price%5B%5D=-45";
      ...
      if (item.title && item.price && item.size) {
        ...
        if (!hasFlaw) {
          if (firstMatch) {
            ...
            await sendTelegramMessage(
              "🔔 *You got new deals!*\n\nHere are the latest Men Dryvent Jackets:"
            );
            firstMatch = false;
          }
